-- Fix: create_store_order was using a non-existent fallback product (kind IS NULL),
-- causing transactions.product_id to be NULL and breaking the not-null constraint.
-- Use the first cart item's product_id as the fallback instead.
...
BEGIN
  ...
  -- fallback product = first cart item's product (always exists in products table)
