-- ============================================================================
-- ETAPA 1: recalc_wallets_for_owner passa a LER commissions.is_network
-- Assinatura/retorno inalterados: (_profile_id uuid) RETURNS void
--
-- ROLLBACK: reexecutar a definicao abaixo trocando
...
--       )
--     ) AS is_network,
-- Nenhuma outra linha da funcao difere da versao anterior.
--
-- NOTA: divergencia coluna x expressao = 0 linhas (verificado). A unica carteira
-- que muda no recalculo (5c570e35-24c0-4060-8547-098a12c62c2f) ja mudava com a
-- funcao ANTIGA: comissao de indicacao de R$ 40,00 criada em 28/07 nunca
-- refletida na carteira. Verificado por execucao de diagnostico abortada.
-- ============================================================================
...
BEGIN
  FOR r IN SELECT id FROM public.profiles LOOP
    PERFORM public.recalc_wallets_for_owner(r.id);
  END LOOP;
END $recalc$;
...
BEGIN
  ...
  JOIN _antes a ON a.profile_id = w.profile_id
  WHERE w.profile_id <> '5c570e35-24c0-4060-8547-098a12c62c2f'::uuid
    AND (ROUND(w.available_balance::numeric,2) <> ROUND(a.available_balance::numeric,2)
     OR ROUND(w.pending_balance::numeric,2)   <> ROUND(a.pending_balance::numeric,2)
     OR ROUND(w.total_earned::numeric,2)      <> ROUND(a.total_earned::numeric,2)
     OR ROUND(w.total_withdrawn::numeric,2)   <> ROUND(a.total_withdrawn::numeric,2));
  IF v_diff > 0 THEN
    RAISE EXCEPTION 'ABORTADO: % carteiras mudariam de valor. Investigar antes de aplicar.', v_diff;
  END IF;
END $guard$;
