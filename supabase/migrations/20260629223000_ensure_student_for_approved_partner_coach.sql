-- Garante que todo parceiro/coach aprovado também tenha perfil de aluno.
-- O motivo do caso Irineia: o fluxo de parceiro aprovado por etapas/admin não chamava
-- a criação de students; além disso o cadastro dela ficou sem upline_coach_id e
-- students.coach_id é obrigatório. Por isso usamos um coach sistema como fallback.
...
-- Backfill: cria alunos faltantes para parceiros aprovados e coaches aprovados existentes.
DO $$
DECLARE
  r record;
BEGIN
