...
-- Admin function to purge a user's public-schema dependents, then the function
-- caller can delete the auth user via admin API (which cascades to profiles).
...
BEGIN
  ...
  -- Generic cleanup: for every FK referencing the affected ids, NULL out
  -- nullable columns and DELETE rows where the referencing column is NOT NULL.
  -- This sidesteps having to hand-enumerate every dependent table.
  FOR r IN
    SELECT
    ...
    WHERE con.contype = 'f'
      ...
      AND con.confdeltype = 'a' -- 'a' = NO ACTION (not cascade/set null)
  LOOP
    DECLARE
      v_id uuid;
    BEGIN
