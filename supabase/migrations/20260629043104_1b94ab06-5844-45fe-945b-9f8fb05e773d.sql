...
-- Helpers de reversão idempotente
...
BEGIN
  ...
  LOOP
    ...
    SELECT date_trunc('month', COALESCE(paid_at, created_at))::DATE INTO v_paid_month
      FROM public.transactions WHERE id = _transaction_id;

    IF v_paid_month IS NOT NULL THEN
      UPDATE public.monthly_rankings
      SET points = GREATEST(0, COALESCE(points,0) - r.pts),
          sales_count = GREATEST(0, COALESCE(sales_count,0) - r.qty),
          revenue = GREATEST(0, COALESCE(revenue,0) -
            COALESCE((SELECT gross_amount FROM public.transactions WHERE id = _transaction_id),0))
      WHERE coach_id = r.coach_id AND month = v_paid_month;
    END IF;
  END LOOP;
END;
$$;

-- Atualização do process_paid_transaction
...
BEGIN
  ...
  -- Reverter pontos/ranking/desafios (idempotência)
  ...
  IF slot_count > 0 THEN
    ...
    LOOP
      ...
      -- Em venda por indicação: destino 'referral_student' vira Fitcoin do indicador
      ...
      ELSIF effective_destination IN ('professor_wallet','professor_blocked') THEN
        -- Resolve professor do produto/curso (course_teacher_commissions ou product)
      ...
      ELSIF effective_destination = 'referral_student' THEN
        -- venda normal (sem indicação): slot dedicado a indicação é ignorado
        total_distributed := total_distributed - slot_amount;
        group_total := group_total - slot_amount;
        CONTINUE;
  ...
  -- Sobra vai SEMPRE para o coach (inclusive em venda por indicação)
