-- 1) Atualiza process_paid_transaction: ranking deve usar GROSS (valor da venda do produto), não net.
...
BEGIN
  ...
  IF coach_row.id IS NOT NULL THEN
    ...
    -- Ranking deve refletir o VALOR DA VENDA DO PRODUTO (gross), não a comissão líquida
...
-- 2) Backfill: recalcula total_revenue de monthly_rankings com base no gross das vendas pagas.
WITH agg AS (
  SELECT
    s.coach_id,
    date_trunc('month', t.paid_at)::DATE AS ref_month,
    SUM(COALESCE(t.gross_amount, 0)) AS gross_total
  FROM public.transactions t
  JOIN public.students s ON s.id = t.student_id
  WHERE t.status = 'paid' AND s.coach_id IS NOT NULL
  GROUP BY s.coach_id, date_trunc('month', t.paid_at)::DATE
)
UPDATE public.monthly_rankings mr
SET total_revenue = agg.gross_total
FROM agg
WHERE mr.coach_id = agg.coach_id
  AND mr.reference_month = agg.ref_month;
