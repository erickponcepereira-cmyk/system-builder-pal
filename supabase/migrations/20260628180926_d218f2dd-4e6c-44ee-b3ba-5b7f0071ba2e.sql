-- 1) Corrige process_paid_transaction:
--    a) Slot "nutricionista" sem nutricionista vinculado vira crédito virtual no admin
--       (slot_label contendo "nutricionista") para aparecer em "Admin Nutricionista (não atribuído)".
--    b) Mantém slot "professor" indo para admin_system_wallet_entries (carteira virtual professor),
--       mas marca consistentemente o slot_label.
...
BEGIN
  ...
  IF slot_count > 0 THEN
    ...
    LOOP
      ...
      IF effective_destination IN ('nutritionist_wallet','nutritionist_blocked') THEN
        IF beneficiary_profile IS NOT NULL THEN
          ...
          END IF;
        ELSE
          -- Sem nutricionista vinculado: crédito virtual no admin para
          -- aparecer em "Admin Nutricionista (não atribuído)" e ser atribuído depois.
          INSERT INTO public.admin_system_wallet_entries (transaction_id, slot_label, amount, kind, notes)
          VALUES (_transaction_id,
                  CASE WHEN slot.label ILIKE '%nutricion%' THEN slot.label ELSE slot.label || ' (nutricionista)' END,
                  slot_amount, 'credit',
                  'Aguardando atribuição a nutricionista');
        END IF;
      ELSIF effective_destination IN ('professor_wallet','professor_blocked') THEN
        -- Crédito virtual no admin (slot_label marcado) para atribuição manual ao professor.
        -- Esse valor é exibido somente na carteira de Professor (filtros de admin excluem "%professor%").
        INSERT INTO public.admin_system_wallet_entries (transaction_id, slot_label, amount, kind, notes)
        VALUES (_transaction_id,
...
-- 2) Backfill da transação f3880bf7-ebf9-4794-bf86-f56a2e334d56 para credit nutricionista R$0,05
INSERT INTO public.admin_system_wallet_entries (transaction_id, slot_label, amount, kind, notes)
SELECT 'f3880bf7-ebf9-4794-bf86-f56a2e334d56'::uuid, 'nutricionista', 0.05, 'credit', 'Aguardando atribuição a nutricionista (backfill)'
WHERE NOT EXISTS (
  SELECT 1 FROM public.admin_system_wallet_entries
  WHERE transaction_id = 'f3880bf7-ebf9-4794-bf86-f56a2e334d56'::uuid
    AND kind = 'credit'
    AND slot_label ILIKE '%nutricion%'
);