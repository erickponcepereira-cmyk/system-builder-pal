-- O retrato passa a levar a data real de vencimento, e o limite da ULTIMA venda.
--
...
BEGIN
  ...
  WITH pessoa AS (
    SELECT cr.referencia,
           ...
           -- Manda o limite da ULTIMA mensalidade VENDIDA, nao a de vencimento
           -- mais longe.
           --
           -- Normalmente da no mesmo, porque renovar soma em cima do que resta.
           -- Mas quando a recepcao escolhe a data na mao (cortesia, acerto de
           -- dias), o plano novo pode ter vencimento menor que um trimestral
           -- ainda em curso -- e pela regra antiga o limite dele era ignorado.
           -- Quem acabou de comprar 3x tem que valer 3x a partir da compra.
           (array_agg(m.limite_dias_semana ORDER BY m.created_at DESC))[1] AS limite
