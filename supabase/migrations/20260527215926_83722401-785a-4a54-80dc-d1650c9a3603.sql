...
-- 1) Make nathan a master admin too
UPDATE public.profiles SET is_master_admin = true WHERE email = 'nathan.utuari@gmail.com';

-- 2) Redirect system fees to the shared admin_system_wallet (instead of an individual master admin's wallet)
    ...
    LOOP
      ...
      CASE effective_destination
        WHEN 'admin_wallet' THEN
          -- Credit shared admin wallet, not an individual profile
...
-- 3) Backfill: migrate existing admin/system commissions into the shared wallet
WITH moved AS (
  DELETE FROM public.commissions
  WHERE (slot_label ILIKE '%sistema%' OR slot_label ILIKE '%admin%')
    AND beneficiary_coach_id IS NULL
  RETURNING transaction_id, slot_label, amount, status
)
INSERT INTO public.admin_system_wallet_entries (transaction_id, slot_label, amount, kind)
SELECT transaction_id, slot_label, amount, 'credit' FROM moved;

UPDATE public.admin_system_wallet
SET available_balance = COALESCE((SELECT SUM(amount) FROM public.admin_system_wallet_entries WHERE kind='credit'),0)
                       - COALESCE((SELECT SUM(amount) FROM public.admin_system_wallet_entries WHERE kind='debit'),0),
    total_earned = COALESCE((SELECT SUM(amount) FROM public.admin_system_wallet_entries WHERE kind='credit'),0),
    total_withdrawn = COALESCE((SELECT SUM(amount) FROM public.admin_system_wallet_entries WHERE kind='debit'),0),
    updated_at = NOW()
WHERE id = true;
