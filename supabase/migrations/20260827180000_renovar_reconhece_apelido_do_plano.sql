-- A renovacao passa a reconhecer o nome antigo do plano.
--
-- `academia_renovar` procurava o plano por nome EXATO para herdar o limite
-- semanal. As 413 mensalidades importadas gravaram "Funcional - Mensal - 3x
-- semana"; o cadastro chama isso de "Mensal - 3x semana". Renovando com o nome
-- antigo, o limite de 3 dias nao era herdado e a pessoa voltava a entrar todo
-- dia -- exatamente o bug que o limite semanal existe para impedir.
--
-- Unica mudanca: o SELECT do limite usa `academia_plano_do_texto`.
...
BEGIN
  ...
  --
  -- Pela busca com apelido, nao por nome exato: quem veio do Next Fit tem o
  -- plano gravado como "Funcional - Mensal - 3x semana", e o cadastro chama
  -- isso de "Mensal - 3x semana". Renovando com o nome antigo -- por script de
  -- importacao ou digitado igual ao que estava la -- o limite de 3 dias nao era
  -- herdado e a pessoa passava a entrar todo dia.
  SELECT pl.limite_dias_semana INTO v_limite
    FROM public.academia_plano_do_texto(p_partner_id, p_plano) pl;
...
-- E a sobrecarga velha some.
--
-- A migration 20260827090000 acrescentou `p_valido_ate` a `academia_renovar`.
-- CREATE OR REPLACE nao substitui uma funcao quando a lista de parametros muda:
-- ela CRIA outra. Ficaram duas, e a de 9 argumentos e a versao anterior --
-- aquela que grava a mensalidade SEM `limite_dias_semana`.
--
-- Ninguem chama a de 9 hoje (o servidor sempre manda p_valido_ate, mesmo que
-- null), entao o estrago nunca aconteceu. Mas quem chamasse com 9 argumentos
-- venderia um plano de 3x sem limite nenhum -- exatamente o que o limite
-- semanal existe para impedir, e sem erro nenhum na tela.
DROP FUNCTION IF EXISTS public.academia_renovar(
  uuid, uuid, uuid, text, numeric, jsonb, integer, uuid, text
);
