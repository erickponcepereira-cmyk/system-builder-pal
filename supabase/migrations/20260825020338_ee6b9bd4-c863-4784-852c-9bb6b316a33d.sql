...
BEGIN
  ...
  -- Quem e o coach da venda.
  -- 1) vendedor registrado na venda (painel de coach), desde que nao seja o proprio comprador
  -- 2) coach responsavel pelo aluno, desde que nao seja o proprio comprador
  -- 3) ultimo recurso: cadastro de coach do proprio comprador (coach sem responsavel distinto)
  SELECT * INTO coach_row FROM public.coaches WHERE false;
  BEGIN
    v_seller_coach_id := NULLIF(tx.metadata->>'created_by_coach_id','')::uuid;
  EXCEPTION WHEN others THEN
    v_seller_coach_id := NULL;
  END;

  IF v_seller_coach_id IS NOT NULL THEN
    SELECT * INTO coach_row
    FROM public.coaches c
    WHERE c.id = v_seller_coach_id
      AND c.blocked_at IS NULL
      AND (student_row.profile_id IS NULL OR c.profile_id IS DISTINCT FROM student_row.profile_id);
  END IF;

  IF coach_row.id IS NULL AND student_row.coach_id IS NOT NULL THEN
    SELECT * INTO coach_row
    FROM public.coaches c
    WHERE c.id = student_row.coach_id
      AND (student_row.profile_id IS NULL OR c.profile_id IS DISTINCT FROM student_row.profile_id);
  END IF;
    ...
    LOOP
      ...
      IF v_is_referral_sale AND slot.destination::TEXT = 'referral_student' THEN
        INSERT INTO public.commissions (
          ...
          slot_amount,
          'pending', v_release_base + (commission_release_days || ' days')::INTERVAL,
          slot.label,
