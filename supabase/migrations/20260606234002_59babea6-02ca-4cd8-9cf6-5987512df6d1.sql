...
BEGIN
  ...
  -- Limpa créditos anteriores desta transação
  ...
  -- Reverte créditos anteriores no admin_system_wallet desta tx
    ...
    LOOP
      ...
      CASE slot.destination
        WHEN 'admin_wallet' THEN
          -- Crédito direto na carteira do SISTEMA. Nunca vira comissão pessoal.
        ...
        WHEN 'network_l1' THEN
          IF upline1_id IS NOT NULL THEN
            ...
            v_level := 1;
          ELSE
            -- Sem upline: vai para o sistema (não para o coach vendedor)
            v_credit_admin_system := TRUE;
          END IF;
...
-- Limpa comissões "Sistema" que viraram crédito pessoal do admin/coach em testes
DELETE FROM public.commissions
WHERE COALESCE(slot_label, '') ~* '^(sistema|admin)';

-- Reprocessa transações pagas dos últimos 7 dias para refletir a nova distribuição
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN
  ...
  LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'reprocess failed for %: %', r.id, SQLERRM;
    END;
  END LOOP;
END $$;
