CREATE OR REPLACE FUNCTION public.admin_merge_profiles(p_source uuid, p_target uuid, p_actor uuid DEFAULT NULL, p_dry_run boolean DEFAULT true)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_src_student IS NOT NULL AND v_tgt_student IS NOT NULL AND v_src_student <> v_tgt_student THEN
    ...
    BEGIN
      ...
      IF NOT p_dry_run THEN
        ...
        EXCEPTION WHEN unique_violation OR check_violation OR foreign_key_violation THEN
          v_skipped := v_skipped || jsonb_build_object(r.tbl || '.' || r.col, v_cnt);
        END;
      ELSE
        v_moved := v_moved || jsonb_build_object(r.tbl || '.' || r.col, v_cnt);
      END IF;
  ...
  IF NOT p_dry_run THEN
    ...
    VALUES (p_actor, p_source, 'merge_profiles',
            format('Conta %s unificada em %s. Movido: %s. Mantido na conta antiga (conflito): %s',
                   v_source.email, v_target.email, v_moved::text, v_skipped::text));
  END IF;
...
REVOKE ALL ON FUNCTION public.admin_merge_profiles(uuid, uuid, uuid, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_merge_profiles(uuid, uuid, uuid, boolean) TO authenticated, service_role;