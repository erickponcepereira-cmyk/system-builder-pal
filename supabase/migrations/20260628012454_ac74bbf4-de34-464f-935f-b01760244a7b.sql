...
BEGIN
  ...
  -- Upline L1: se não houver upline, o valor vai para a carteira NORMAL do vendedor (level 0), não como comissão de rede
...
-- Reclassifica comissões de rede (level 1/2/3) que foram creditadas ao próprio vendedor por falta de upline,
-- convertendo-as em comissão direta (level 0).
UPDATE public.commissions
SET level = 0,
    slot_label = CASE
      WHEN level = 1 THEN 'Comissão Direta (sem upline N1)'
      WHEN level = 2 THEN 'Comissão Direta (sem upline N2)'
      WHEN level = 3 THEN 'Comissão Direta (sem upline N3)'
      ELSE slot_label
    END
WHERE partner_order_id IS NOT NULL
  ...
  AND EXISTS (
    ...
    WHERE o.id = commissions.partner_order_id
      AND (
        (commissions.level = 1 AND o.upline_l1_coach_id IS NULL AND commissions.beneficiary_coach_id = o.selling_coach_id) OR
        (commissions.level = 2 AND o.upline_l2_coach_id IS NULL AND commissions.beneficiary_coach_id = o.selling_coach_id) OR
        (commissions.level = 3 AND o.upline_l3_coach_id IS NULL AND commissions.beneficiary_coach_id = o.selling_coach_id)
      )
  );
