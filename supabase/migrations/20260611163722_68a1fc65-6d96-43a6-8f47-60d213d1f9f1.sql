...
-- Fix partner / professional / company product order creation to use:
--   * CASCADE fee calculation (matches the in-app simulator in partnerFinance.ts)
--   * Configured coach commission percentage from the product
--   * Fixed system fee % (5) and tax (6) computed in cascade (not gross-only)
--   * Network L1/L2/L3 = 3/2/1% of the COACH COMMISSION (not gross)
--   * Master Coach cross-bonus uses the beneficiary's configured pct (10..70)
-- The processing function already falls back missing uplines to the selling
-- coach, so when there is no upline the network amounts go to the seller.
...
BEGIN
  ...
  -- CASCADE calculation, matching src/lib/partnerFinance.ts
  ...
  -- Network = % of coach commission (not of gross)
  ...
  -- Master Coach cross-sale bonus: configurable %, taken from partner share
...
BEGIN
  ...
  SELECT pp.id, pp.partner_id, pp.price, pp.status, pp.is_active_by_partner, pp.kind,
         pp.coach_commission_percentage
    INTO v_prod
