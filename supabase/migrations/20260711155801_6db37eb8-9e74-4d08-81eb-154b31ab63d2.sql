...
BEGIN
  ...
  IF v_student_id IS NULL THEN
    SELECT s.id, s.coach_id INTO v_student_id, v_student_coach_id FROM public.students s WHERE s.profile_id = public.current_profile_id();
  ELSE
    SELECT s.coach_id INTO v_student_coach_id FROM public.students s WHERE s.id = v_student_id;
  END IF;
