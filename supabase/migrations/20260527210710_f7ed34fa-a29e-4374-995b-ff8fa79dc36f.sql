...
-- =============================================================
-- 1) Função find_hbl_coach_for: busca coach HBL na rede (upline)
--    Prioriza badge coach_hbl_42, depois coach_hbl_50.
-- =============================================================
CREATE OR REPLACE FUNCTION public.find_hbl_coach_for(_coach_id UUID)
RETURNS UUID
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_found UUID;
BEGIN
  -- 1ª tentativa: HBL 42% subindo na rede
  v_found := public.find_upline_with_badge(_coach_id, 'coach_hbl_42'::public.coach_badge_key);
  IF v_found IS NOT NULL THEN RETURN v_found; END IF;
  -- 2ª: HBL 50% subindo na rede
  v_found := public.find_upline_with_badge(_coach_id, 'coach_hbl_50'::public.coach_badge_key);
  IF v_found IS NOT NULL THEN RETURN v_found; END IF;
  -- Fallback: qualquer coach que tenha um dos badges (qualquer nível)
  SELECT cb.coach_id INTO v_found
  FROM public.coach_badges cb
  WHERE cb.badge_key = 'coach_hbl_42'
  LIMIT 1;
  IF v_found IS NOT NULL THEN RETURN v_found; END IF;
  SELECT cb.coach_id INTO v_found
  FROM public.coach_badges cb
  WHERE cb.badge_key = 'coach_hbl_50'
  LIMIT 1;
  RETURN v_found;
END;
$$;
...
-- =============================================================
-- 2) Coluna hbl_fulfiller_coach_id em product_order_pool_entries
-- =============================================================
ALTER TABLE public.product_order_pool_entries
  ADD COLUMN IF NOT EXISTS hbl_fulfiller_coach_id UUID REFERENCES public.coaches(id);

CREATE INDEX IF NOT EXISTS idx_pool_hbl_fulfiller
  ON public.product_order_pool_entries(hbl_fulfiller_coach_id);

-- =============================================================
-- 3) Atualiza process_paid_transaction para popular hbl_fulfiller
--    no insert do pool. (replicação completa da função)
-- =============================================================
...
BEGIN
  ...
  -- Resolve coach HBL responsável pelo envio (por proximidade de rede)
  v_hbl_coach_id := NULL;
  IF coach_row.id IS NOT NULL THEN
