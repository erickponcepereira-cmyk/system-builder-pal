...
RETURNS TABLE(valid boolean, kind text, sponsor_name text, coach_id uuid, referred_by_student_id uuid)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  IF FOUND THEN
    RETURN QUERY SELECT true, 'coach'::text, _coach.coach_name::text, _coach.coach_id, NULL::uuid;
    RETURN;
  END IF;
  ...
  IF FOUND THEN
    RETURN QUERY SELECT true, 'student'::text, _student.student_name::text, _student.coach_id, _student.student_id;
    RETURN;
  END IF;
