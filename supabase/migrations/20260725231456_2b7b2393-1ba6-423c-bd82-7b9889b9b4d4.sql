...
BEGIN
  ...
  IF v_student_id IS NULL THEN
    ...
    VALUES (v_profile_id, v_fallback_coach, v_ref_code, '/i/' || v_ref_code)
  ...
  SELECT r.id, r.qr_token, r.slot_end
    INTO v_existing_id, v_existing_token, v_existing_end
  FROM public.partner_freebie_reservations r
  WHERE r.partner_product_id = _product_id
    AND r.student_id = v_student_id
    AND r.slot_start = _slot_start
  ...
  IF v_existing_id IS NOT NULL THEN
    ...
    qr_token := v_existing_token;
    slot_end := v_existing_end;
    RETURN NEXT;
    RETURN;
  END IF;
