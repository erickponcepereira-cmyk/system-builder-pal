...
BEGIN
  ...
  -- Safely resolve uplines (avoid accessing not-yet-assigned RECORDs)
    ...
    LOOP
      ...
      CASE slot.destination
        ...
        WHEN 'network_l1'          THEN
          IF upline1_id IS NOT NULL THEN beneficiary_profile := upline1.profile_id; END IF;
        WHEN 'network_l2'          THEN
          IF upline2_id IS NOT NULL THEN beneficiary_profile := upline2.profile_id; END IF;
        WHEN 'network_l3'          THEN
          IF upline3_id IS NOT NULL THEN beneficiary_profile := upline3.profile_id; END IF;
