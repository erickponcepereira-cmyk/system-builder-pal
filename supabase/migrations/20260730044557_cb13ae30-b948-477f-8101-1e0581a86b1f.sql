...
BEGIN
  ...
  -- Estoque / vagas: conta pedidos vivos (pendentes e pagos) do produto.
  IF v_prod.stock IS NOT NULL THEN
    SELECT count(*) INTO v_used
      FROM public.partner_product_orders o
     WHERE o.partner_product_id = v_prod.id
...
CREATE OR REPLACE FUNCTION public.partner_products_stock_status(_ids uuid[])
 RETURNS TABLE (product_id uuid, stock int, used int, remaining int)
 LANGUAGE sql
 STABLE
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  SELECT pp.id,
         ...
         GREATEST(0, pp.stock - COALESCE(o.cnt, 0))::int
    FROM public.partner_products pp
    LEFT JOIN (
      SELECT partner_product_id, count(*)::int AS cnt
        FROM public.partner_product_orders
       WHERE COALESCE(status, '') NOT IN ('cancelled','refunded','failed')
       GROUP BY partner_product_id
    ) o ON o.partner_product_id = pp.id
   WHERE pp.id = ANY(_ids)
     AND pp.stock IS NOT NULL;
$function$;

GRANT EXECUTE ON FUNCTION public.partner_products_stock_status(uuid[]) TO anon, authenticated;