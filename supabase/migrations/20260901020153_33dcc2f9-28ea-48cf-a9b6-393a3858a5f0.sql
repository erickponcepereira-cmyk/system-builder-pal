...
DECLARE
  ...
  v_creator_released numeric := 0;
  v_released_total numeric := 0;

  v_wd_paid numeric := 0;
  v_wd_open numeric := 0;
  v_prof_wd_paid numeric := 0;
  v_prof_wd_open numeric := 0;
  v_spent numeric := 0;
  v_advance numeric := 0;
...
BEGIN
  ...
  v_prof_wd_paid := v_wd_paid;
  v_prof_wd_open := v_wd_open;
  ...
  -- Liberado de verdade: comissões liberadas + ganho de criador fora da carência.
  v_creator_released := GREATEST(v_creator_earned - v_creator_pend, 0);
  v_released_total := v_comm_released + v_creator_released;

  -- Consolidado entre carteiras: o excesso pago em uma abate a outra.
  v_net := v_released_total - v_prof_wd_paid - v_prof_wd_open - v_spent;
  v_base_avail := GREATEST(v_net, 0);
  v_overpaid := GREATEST(-v_net, 0);

  RETURN jsonb_build_object(
    'profile_id', _profile_id,
    'generated_at', now(),
    -- adiantamento em aberto ainda desconta; o já quitado não desconta de novo.
    'available', round(GREATEST(v_base_avail - v_advance, 0)::numeric, 2),
    'available_before_advance', round(v_base_avail::numeric, 2),
    'overpaid', round(v_overpaid::numeric, 2),
    'released_total', round(v_released_total::numeric, 2),
