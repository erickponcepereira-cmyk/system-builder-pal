...
export function WalletTab() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [commRes, recentWithdrawsRes] = await Promise.all([
        supabase.from("commissions").select("id,amount,level,created_at,transaction_id,slot_label").eq("beneficiary_profile_id", profile.id).order("created_at", { ascending: false }).limit(20),
