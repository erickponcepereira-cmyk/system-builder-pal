...
type Profile = { name: string; email: string; phone: string | null; birthdate: string | null; city: string | null; state: string | null; photo_url?: string | null; avatar_url?: string | null };
  ...
  useEffect(() => {
    (async () => {
      ...
      const [subRes, txRes, bodyRes, bioRes, anamRes, wRes, pRes] = await Promise.all([
        ...
        supabase.from("anamnesis_forms").select(ANAMNESIS_COLUMNS).eq("student_id", studentId).order("filled_at", { ascending: false }),
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading ? (
            ...
            <div className="space-y-3">
              ...
              <Card title="Tickets de Desafio" icon={<Coins className="h-4 w-4 text-primary" />}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-white/50">Cada ticket = 1 entrada em 1 desafio</p>
            ...
            <div className="space-y-3">
              {anams.length === 0 ? (
                ...
                <AnamnesisView key={a.id} row={a as unknown as AnamnesisRow} />
              ))}
            </div>
          ) : tab === "janelas" ? (
            <div className="space-y-4">
