...
export function StudentEvaluationPanel() {
  ...
  return (
    <div className="space-y-5 text-foreground">
      <div className="rounded-2xl border border-foreground/10 bg-[#1A1A1A] p-5">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            <h2 className="text-lg font-bold text-foreground">Avaliar Aluno</h2>
            <p className="text-sm text-foreground">Bioimpedância com classificação em tempo real</p>
          </div>
          ...
          {fields.map((field) => (
            <div key={field.label} className="space-y-2">
              ...
              <div className="relative">
                <field.icon className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-foreground" />
                <Input
                  value={field.value}
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <div key={card.label} className="rounded-2xl border border-foreground/10 bg-[#1A1A1A] p-4">
            ...
            <p className="mt-1 text-[11px] text-foreground">{card.extra}</p>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-2xl border border-foreground/10 bg-[#1A1A1A] p-5">
        ...
        <button
          type="button"
          className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-foreground/20 bg-foreground/5 px-4 py-6 text-sm font-medium text-foreground transition-colors hover:border-primary/50 hover:bg-foreground/10"
        >
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {["Objetivo do protocolo", "Condições pré-existentes", "Medicamentos atuais", "Rotina de exercícios"].map((label) => (
            <label key={label} className="space-y-2">
              ...
              <textarea className="min-h-20 w-full rounded-xl border border-foreground/10 bg-foreground/50 p-3 text-sm text-foreground outline-none focus:border-primary/40" />
            </label>
          ))}
        </div>
