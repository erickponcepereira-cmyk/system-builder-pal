...
export function WalletStatementCard({
  ...
  const rows: Array<{ label: string; value: number; hint?: string; tone?: string; subs?: Array<{ label: string; value: number }> }> = [
      ...
      : []),
    {
      ...
      tone: "text-amber-400",
      hint: "Aguardando o prazo de liberação — não inclui rede",
      subs: [
        { label: "Coach (comissões)", value: s.coach.hold },
        { label: "Parceiro (produtos/serviços)", value: s.partner.hold },
        { label: "Profissional", value: s.professional.hold },
      ],
    },
    { label: "Rede bloqueada", value: statement.networkBlocked, tone: "text-amber-400", hint: "Libera ao bater a missão do mês — não entra no a liberar" },
