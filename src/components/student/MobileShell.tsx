...
import { ClipboardList, Home, Trophy, MessageCircle, ShoppingBag, User, Gift, BookOpen, Camera } from "lucide-react";
...
const navItems = [
  ...
  { to: "/student/library", label: "Cursos", icon: BookOpen },
...
export function MobileShell({ children }: MobileShellProps) {
  ...
  return (
    <div className="min-h-screen w-full flex justify-center" style={{ backgroundColor: "#0A0A0A" }}>
      {/* Mobile container 430px max */}
      <div
        className="relative flex w-full max-w-[430px] flex-col min-h-screen shadow-2xl"
      ...
      >
        ...
        >
          <div className="grid grid-cols-9 px-1 py-2 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
            {navItems.map((item) => {
              const isActive =
                item.to === "/student"
