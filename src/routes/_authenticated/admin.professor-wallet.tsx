...
import {
  listProfessorWallets,
  listProfessorBlockedEntries,
  releaseProfessorEntry,
  cancelProfessorEntry,
  assignProfessorToSystemEntry,
  type ProfessorWalletRow,
  type ProfessorBlockedEntry,
} from "@/lib/professor.functions";

export const Route = createFileRoute("/_authenticated/admin/professor-wallet")({
  head: () => ({ meta: [{ title: "Carteira do Professor — Admin" }] }),
  component: ProfessorWalletPage,
});

const money = (v: number) =>
  `R$ ${v.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;

function ProfessorWalletPage() {
  const fetchWallets = useServerFn(listProfessorWallets);
  const fetchEntries = useServerFn(listProfessorBlockedEntries);
  const releaseFn = useServerFn(releaseProfessorEntry);
  const cancelFn = useServerFn(cancelProfessorEntry);
  const assignFn = useServerFn(assignProfessorToSystemEntry);

  const [wallets, setWallets] = useState<ProfessorWalletRow[] | null>(null);
  const [entries, setEntries] = useState<ProfessorBlockedEntry[] | null>(null);
  const [filter, setFilter] = useState<"blocked" | "released" | "cancelled" | "all">("blocked");
  const [busy, setBusy] = useState<string | null>(null);
  const [assignFor, setAssignFor] = useState<ProfessorBlockedEntry | null>(null);
  ...
  const release = async (e: ProfessorBlockedEntry) => {
    if (!confirm(`Liberar ${money(e.amount)} para ${e.profile_name}?\n\nMove o valor para o saldo disponível do professor.`))
      return;
    setBusy(e.id);
    try {
  ...
  const cancel = async (e: ProfessorBlockedEntry) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-white">Carteira do Professor</h1>
        <p className="text-xs text-white/50">
          Valores destinados a "Professor" ficam <strong>aguardando atribuição</strong> a um professor específico. Após
          atribuir, o saldo fica bloqueado no professor até a liberação manual.
        </p>
      </div>
                ...
                {entries.map((e) => (
                  <tr key={e.id} className="border-t border-white/5">
                    ...
                    <td className="p-3 text-right">
                      ...
                      {e.status === "blocked" && e.source === "professor" && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={() => setAssignFor(null)}>
          <div className="w-full max-w-md rounded-xl border border-white/10 bg-[#0F0F0F] p-5" onClick={(e) => e.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-base font-bold text-white">Atribuir professor</h2>
            ...
            <label className="mt-4 block text-xs font-semibold uppercase text-white/50">Professor</label>
            <select
              value={assignTarget}
            ...
            >
              ...
              {(wallets || []).filter((w) => w.profile_id !== "admin-professor").map((w) => (
                <option key={w.profile_id} value={w.profile_id}>{w.name}</option>
              ))}
            </select>
            <p className="mt-2 text-[11px] text-white/40">
              Para que um perfil apareça aqui, crie uma carteira de professor para ele (basta uma primeira atribuição via crédito virtual). Em breve será possível vincular professores diretamente pelo painel.
            </p>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    toast.success("Venda atribuída ao professor");
                    setAssignFor(null);
                    reload();
                  } catch (err: any) {
                    toast.error(err.message || "Erro");
