...
type ApplicationRow = {
  ...
  selected_upline_name?: string | null;
  motivation: string;
...
function CoachApplicationsPage() {
  ...
  const load = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select("id,selected_upline_coach_id,motivation,experience,city,phone,status,completed_modules,total_modules,admin_notes,created_at,profiles!coach_applications_profile_id_fkey(name,email)" as never)
      .order("created_at" as never, { ascending: false });
    if (error) toast.error(error.message);
    const rows = (data as unknown as ApplicationRow[]) || [];
    const coachIds = [...new Set(rows.map((item) => item.selected_upline_coach_id).filter(Boolean))] as string[];
    const { data: coaches } = coachIds.length
      ? await supabase.from("coaches" as never).select("id,profiles!coaches_profile_id_fkey(name)" as never).in("id" as never, coachIds as never)
      : { data: [] };
    const coachNames = new Map(((coaches as unknown as Array<{ id: string; profiles?: { name?: string | null } | null }>) || []).map((coach) => [coach.id, coach.profiles?.name || "Coach selecionado"]));
    setItems(rows.map((item) => ({ ...item, selected_upline_name: item.selected_upline_coach_id ? coachNames.get(item.selected_upline_coach_id) || null : null })));
    setLoading(false);
  };
