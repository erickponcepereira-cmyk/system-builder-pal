...
function AdminOrders() {
  ...
  return <>
    <TestModeBanner hiddenLabel="Pedidos" />
    ...
    {loading ? <div className="rounded-2xl p-10 text-center text-white/50" style={{ backgroundColor: "#1A1A1A" }}><Loader2 className="mx-auto mb-3 h-6 w-6 animate-spin text-primary" />Carregando pedidos...</div> : filtered.length === 0 ? <div className="rounded-2xl p-10 text-center text-white/50" style={{ backgroundColor: "#1A1A1A" }}>Nenhum pedido encontrado.</div> : <div className="space-y-4">{filtered.map((order) => { const nut = nutByOrder[order.id]; return <article key={order.id} className="rounded-2xl border border-white/5 p-5" style={{ backgroundColor: "#1A1A1A" }}><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><div className="flex items-center gap-2"><PackageCheck className="h-5 w-5 text-primary" /><h2 className="font-bold text-white">{order.order_number}</h2><span className="rounded-full bg-primary/15 px-2 py-0.5 text-[10px] font-bold text-primary">{labels[order.status] || order.status}</span></div><p className="mt-1 text-sm text-white/55">{order.students?.profiles?.name || "Aluno"} · {order.students?.profiles?.email}</p><p className="text-xs text-white/40">{order.shipping_city || "sem entrega"}{order.shipping_state ? `/${order.shipping_state}` : ""} · {order.created_at ? new Date(order.created_at).toLocaleString("pt-BR") : "—"}</p></div><div className="text-right"><p className="text-xl font-bold text-white">{fmt(Number(order.total_amount || 0))}</p><p className="text-xs text-white/40">{order.payment_method}</p></div></div><div className="mt-4 grid gap-2 border-t border-white/5 pt-4">{(order.store_order_items || []).map((item, index) => <div key={`${order.id}-${index}`} className="flex items-center justify-between rounded-xl bg-white/5 px-3 py-2 text-xs"><span className="text-white/70">{item.quantity}x {item.title}</span><b className="text-white">{fmt(Number(item.total_price || 0))}</b></div>)}</div>
      <div className="mt-4 flex flex-col gap-2 rounded-xl border border-white/5 bg-white/[0.02] p-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2 text-xs">
          <UserCog className="h-4 w-4 text-primary" />
          <span className="text-white/60">Nutricionista:</span>
          {nut ? (
            <span className="font-medium text-white">{nut.name}{nut.isManualOverride && <span className="ml-2 rounded bg-primary/20 px-1.5 py-0.5 text-[9px] text-primary">manual</span>}</span>
          ) : (
            <span className="text-white/40">não atribuído</span>
          )}
        </div>
        {editingNutOrder === order.id ? (
          <div className="flex items-center gap-2">
            <select value={pickValue} onChange={(e) => setPickValue(e.target.value)} className="rounded-md border border-white/10 bg-white/5 px-2 py-1 text-xs text-white">
              <option value="">Selecione...</option>
              {nutPartners.map((n) => <option key={n.id} value={n.id}>{n.name}</option>)}
            </select>
            <Button size="sm" disabled={savingNut || !pickValue} onClick={() => saveOverride(order.id)}>{savingNut ? "..." : "Salvar"}</Button>
            <Button size="sm" variant="outline" className="border-white/10 text-white/70" onClick={() => { setEditingNutOrder(null); setPickValue(""); }}>Cancelar</Button>
          </div>
        ) : (
          <Button size="sm" variant="outline" className="border-white/10 text-white/70" onClick={() => { setEditingNutOrder(order.id); setPickValue(nut?.nutritionistCoachId || ""); }}>{nut ? "Trocar" : "Atribuir"}</Button>
        )}
      </div>
