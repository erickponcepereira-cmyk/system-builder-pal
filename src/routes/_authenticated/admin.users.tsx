...
import { Loader2, Search, ShieldCheck, ShieldOff, History, Settings2, Crown, MailCheck, Merge, Users } from "lucide-react";
...
import { DuplicateAccountsPanel } from "@/components/admin/DuplicateAccountsPanel";
...
function AdminUsersPage() {
  ...
  const [tab, setTab] = useState<"users" | "merge" | "duplicates" | "audit">("users");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b border-white/5">
        ...
        <TabBtn active={tab === "duplicates"} onClick={() => setTab("duplicates")} icon={<Users className="h-4 w-4" />}>Duplicados</TabBtn>
        <TabBtn active={tab === "audit"} onClick={() => setTab("audit")} icon={<History className="h-4 w-4" />}>Auditoria</TabBtn>
      </div>

      {tab === "merge" && <MergeAccountsPanel />}

      {tab === "duplicates" && <DuplicateAccountsPanel />}
