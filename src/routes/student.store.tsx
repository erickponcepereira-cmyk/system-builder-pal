...
function StorePage() {
  ...
  return (
    <div className="flex flex-col gap-4 p-4 pb-6">
      ...
      <div className="grid grid-cols-2 gap-3">{filtered.map((item) => <button key={item.id} onClick={() => addToCart(item)} className="rounded-2xl bg-card p-3 text-left transition-colors hover:bg-accent"><div className="mb-3 flex aspect-square items-center justify-center rounded-xl bg-muted"><ShoppingBag className="h-8 w-8 text-muted-foreground" /></div>{item.tag && <span className="mb-1 inline-block rounded-full bg-primary/20 px-2 py-0.5 text-[9px] font-bold text-primary">{item.tag}</span>}<p className="min-h-[32px] text-xs font-medium text-foreground line-clamp-2">{item.title}</p>{item.subtitle && <p className="mt-1 line-clamp-2 text-[10px] text-muted-foreground">{item.subtitle}</p>}<div className="mt-1 flex flex-wrap items-baseline gap-1.5"><span className="text-sm font-bold text-foreground">{priceLabel(item)}</span>{item.originalPrice && <span className="text-[10px] text-muted-foreground line-through">{fmt(item.originalPrice)}</span>}</div>{item.kind === "store" && <p className="mt-1 text-[10px] text-muted-foreground">Estoque: {item.stock ?? 0}</p>}</button>)}</div>
