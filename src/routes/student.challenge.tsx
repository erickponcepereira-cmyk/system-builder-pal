...
function ChallengePage() {
  ...
  return <div className="flex flex-col gap-4 p-4 pb-6">
    ...
    <div><h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-white/40">Próximas atividades</h2><div className="space-y-2">{[{ icon: Calendar, title: "Aula do desafio", to: "/student/group" }, { icon: Scale, title: "Pesagem semanal", to: "/student/health" }, { icon: Camera, title: "Foto de evolução", to: "/student/evolution" }].map((it) => <Link key={it.title} to={it.to} className="flex items-center gap-3 rounded-2xl p-3" style={{ backgroundColor: "#1A1A1A" }}><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/15"><it.icon className="h-4 w-4 text-primary" /></div><div className="flex-1"><p className="text-sm font-medium text-white">{it.title}</p><p className="text-[11px] text-white/40">Acessar agora</p></div></Link>)}</div></div>
  </div>;
}
