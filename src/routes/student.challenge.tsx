...
function ChallengePage() {
  ...
  return <div className="flex flex-col gap-4 p-4 pb-6">
    ...
    {loading ? <div className="rounded-2xl p-8 text-center text-white/50" style={{ backgroundColor: "#1A1A1A" }}><Loader2 className="mx-auto mb-2 h-5 w-5 animate-spin text-primary" />Carregando desafio...</div> : <>
      ...
      <div className="rounded-2xl p-4" style={{ backgroundColor: "#1A1A1A" }}><div className="mb-4 flex items-center justify-between"><div><h2 className="text-sm font-bold text-white">Ranking por categoria</h2><p className="text-[11px] text-white/40">Baseado nas avaliações da bioimpedância</p></div><Medal className="h-5 w-5 text-primary" /></div><div className="grid grid-cols-3 gap-2 text-center"><Metric label="Gordura" value={latest?.fat_percentage ? `${latest.fat_percentage}%` : "—"} /><Metric label="Músculo" value={latest?.muscle_percentage ? `${latest.muscle_percentage}%` : "—"} /><Metric label="Peso" value={latest?.weight ? `${latest.weight} kg` : "—"} /></div></div>
    </>}
