  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowFeeConfig(true)} className="flex items-center gap-2 rounded-lg bg-white/5 hover:bg-white/10 px-3 py-2 text-xs text-white">
              ...
              {rows.map((r) => (
                <tr key={r.id} className="border-t border-white/5 text-white">
                  ...
                  <td className="px-4 py-2 text-right">
                    <button onClick={() => setSelectedId(r.id)} className="rounded-md bg-primary/20 hover:bg-primary/30 text-primary px-3 py-1 text-xs font-bold">
                      Editar Financeiro
                    </button>
                  </td>
                </tr>
...
// ─── Drawer: Configurador Financeiro de Produto ────────────────
          ...
          <>
            ...
            <div className="flex items-center gap-3 pb-4 mb-6 border-b border-white/10">
              ...
              <div className="flex-1">
                <div className="text-[15px] font-medium">{data.product.name}</div>
                <div className="text-[12px] text-white/50">Configurador financeiro — fluxo em tempo real</div>
              </div>
              <button onClick={onClose} className="text-white/60 hover:text-white text-sm flex items-center gap-1"><X className="h-4 w-4" /> Fechar</button>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-6">
              {PAYMENT_METHODS.map((m) => {
                ...
                return (
                  <button key={m.id} onClick={() => setPreviewMethod(m.id)}
    ...
    <div className={`rounded-md border-l-2 ${m.borderClass} border border-white/10 p-3`} style={{ backgroundColor: "#1A1A1A" }}>
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={onRemove} className="rounded-md border border-white/10 hover:border-[#E24B4A] hover:text-[#E24B4A] p-1.5 text-white/60">
...
function Toggle({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button onClick={() => onChange(!checked)} className={`w-8 h-[18px] rounded-full relative transition ${checked ? "bg-[#E24B4A]" : "bg-white/15"}`}>
...
function FeeConfigDrawer({ onClose }: { onClose: () => void }) {
  ...
  const handleSave = async (row: FeeConfigRow) => {
    setSaving(row.id);
    try { await upsert({ data: row }); toast.success("Salvo"); }
    catch (e: any) { toast.error(e?.message || "Erro"); }
    finally { setSaving(null); }
  };
  const addNew = async () => {
    try {
      await upsert({ data: { name: "Nova config", card_fee_percentage: 4.98, card_fee_3x12_percentage: 4.98, pix_fee_percentage: 0.99, is_active: true, is_default: false } as Partial<FeeConfigRow> });
      list().then(setRows);
    } catch (e: any) { toast.error(e?.message || "Erro"); }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/70" onClick={onClose}>
      <div className="w-full max-w-xl h-full overflow-y-auto border-l border-white/10 p-6" style={{ backgroundColor: "#0F0F0F" }} onClick={(e) => e.stopPropagation()}>
