...
export const Route = createFileRoute("/coach/partners")({
  head: () => ({ meta: [{ title: "Aprovar Parceiros — Coach" }] }),
  component: CoachPartners,
});

interface PartnerRow {
  id: string; fantasy_name: string; document: string | null; whatsapp: string | null;
  city: string | null; state: string | null; status: string; photo_url: string | null; description: string | null;
}
interface ProductRow {
  id: string; partner_id: string; name: string; kind: string; status: string; price: number;
  image_url: string | null; admin_notes: string | null; partners?: { fantasy_name: string } | null;
}

function CoachPartners() {
  ...
  const updatePartner = async (id: string, patch: Partial<PartnerRow>) => {
    const finalPatch: Partial<PartnerRow> & { approved_at?: string | null; blocked_at?: string | null; blocked_reason?: string | null } = { ...patch };
    if (patch.status === "approved") finalPatch.approved_at = new Date().toISOString();
    if (patch.status === "blocked") finalPatch.blocked_at = new Date().toISOString();
    const { error } = await supabase.from("partners" as never).update(finalPatch as never).eq("id" as never, id);
  ...
  if (allowed === false) {
    return (
      <div className="min-h-screen flex items-center justify-center text-white" style={{ backgroundColor: "#0A0A0A" }}>
        Acesso restrito a coaches e admins.
      </div>
    );
  }
  if (allowed === null) {
  ...
  return (
    <div className="min-h-screen p-4 lg:p-8" style={{ backgroundColor: "#0A0A0A" }}>
      <div className="mx-auto max-w-5xl">
        <Link to="/coach" className="inline-flex items-center gap-2 text-sm text-white/60 hover:text-white mb-4">
          <ArrowLeft className="h-4 w-4" /> Voltar ao painel
        </Link>

        <div className="mb-4">
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <Building2 className="h-6 w-6 text-primary" /> Empresas Parceiras
          </h1>
          <p className="text-sm text-white/50">Coaches e admins podem aprovar empresas e produtos parceiros</p>
        </div>

        <div className="flex gap-2 border-b border-white/10 mb-4">
          <button onClick={() => setTab("empresas")} className={`px-4 py-2 text-sm border-b-2 ${tab === "empresas" ? "border-primary text-primary" : "border-transparent text-white/60"}`}>
            Empresas ({partners.length})
          </button>
          <button onClick={() => setTab("produtos")} className={`px-4 py-2 text-sm border-b-2 ${tab === "produtos" ? "border-primary text-primary" : "border-transparent text-white/60"}`}>
            Produtos pendentes ({products.filter(p => p.status === "pending").length})
          </button>
        </div>
          ...
          <div className="grid gap-2 lg:grid-cols-2">
            {partners.map(p => (
              <div key={p.id} className="rounded-xl p-4 flex gap-3" style={{ backgroundColor: "#1A1A1A" }}>
                {p.photo_url ? <img src={p.photo_url} alt={p.fantasy_name} className="h-14 w-14 rounded-full object-cover" /> : <div className="h-14 w-14 rounded-full bg-white/5" />}
        ...
        {!loading && tab === "produtos" && (
          <div className="space-y-2">
            {products.filter(p => p.status === "pending").map(p => (
              <div key={p.id} className="rounded-xl p-4 flex gap-3" style={{ backgroundColor: "#1A1A1A" }}>
                {p.image_url ? <img src={p.image_url} alt={p.name} className="h-20 w-20 rounded object-cover" /> : <div className="h-20 w-20 rounded bg-white/5" />}
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2 flex gap-2">
                    <button onClick={() => reviewProduct(p.id, "approved")} className="flex items-center gap-1 rounded bg-green-500/15 text-green-400 px-3 py-1.5 text-xs">
                      <Check className="h-3.5 w-3.5" /> Aprovar
                    </button>
                    <button onClick={() => { if (!notes[p.id]?.trim()) return toast.error("Informe a observação"); reviewProduct(p.id, "rejected"); }} className="flex items-center gap-1 rounded bg-red-500/15 text-red-400 px-3 py-1.5 text-xs">
