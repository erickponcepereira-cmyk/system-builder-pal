...
export const getOrCreateActivationOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const activationOrders = ((existingOrders || []) as Array<{
      ...
      item.product_id === ACTIVATION_PRODUCT_ID ||
      item.store_product_id === ACTIVATION_PRODUCT_ID ||
      item.digital_product_id === ACTIVATION_PRODUCT_ID
    ));
