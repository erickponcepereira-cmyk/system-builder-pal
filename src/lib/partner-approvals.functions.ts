...
export const listPartnersForApproval = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabaseAdmin
      .from("partners")
      .select(
        "id, profile_id, fantasy_name, document, whatsapp, city, state, status, photo_url, cover_url, description, instagram, facebook, website, business_area, specialty, created_at, approved_at, profiles:profiles!partners_profile_id_fkey(name, email)",
      )
